...
public:
    ...
    void insert(const LaRankPattern& pattern)
    {
        if (!isPattern(pattern.x_id)) {
            if (freeidx.size()) {
                std::hash_set<unsigned>::iterator it = freeidx.begin();
                patterns[*it] = pattern;
...
private:
    std::hash_set<unsigned> freeidx;
    std::vector<LaRankPattern> patterns;
    std::hash_map<int, unsigned> x_id2rank;
};
...
private:
    ...
    typedef std::hash_map<int, LaRankOutput> outputhash_t; // class index -> LaRankOutput
