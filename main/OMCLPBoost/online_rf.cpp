...
RandomTest::RandomTest(const int& numClasses, const int& numFeatures, const VectorXd &minFeatRange, const VectorXd &maxFeatRange) :
    ...
    m_feature = randDouble(0, numFeatures-1);		// +1-->-1;fixed!确保生成的随机数在0~numFeatures-1之间，有numFeatures个
...
//////////////////////////////////////////////////////////////////////////
void RandomTest::RecordModel(FILE *fp)
{
	fprintf(fp, "%d\t%lf\t%lf\t%lf\n", m_feature, m_threshold, m_trueCount, m_falseCount);
	for (int i = 0; i < *m_numClasses; ++ i)
	{
		fprintf(fp, "%lf\t%lf\n", m_trueStats(i), m_falseStats(i));
	}
	fprintf(fp, "\n");
}

void RandomTest::LoadModel(FILE *fp)
{
	fscanf(fp, "%d%lf%lf%lf", &m_feature, &m_threshold, &m_trueCount, &m_falseCount);
	for (int i = 0; i < *m_numClasses; ++ i)
	{
		double vt, vf;
		fscanf(fp, "%lf%lf", &vt, &vf);
		m_trueStats(i) = vt; 
		m_falseStats(i) = vf;
	}
}
...
//////////////////////////////////////////////////////////////////////////
void OnlineNode::RecordModel(FILE *fp)
{
	fprintf(fp, "%d\t%d\t%d\t", m_depth, m_isLeaf ,m_label);
	fprintf(fp, "%lf\t%lf\n", m_counter, m_parentCounter);
	for (int i = 0; i < *m_numClasses; ++ i)
	{
		fprintf(fp, "%lf\t", m_labelStats(i));
	}
	fprintf(fp, "\n");

	if (!m_isLeaf) {
		m_bestTest->RecordModel(fp);
		m_rightChildNode->RecordModel(fp);
		m_leftChildNode->RecordModel(fp);
	} else {
		for (int nTest = 0; nTest < m_hp->numRandomTests; nTest++) {
			m_onlineTests[nTest]->RecordModel(fp);
		}
	}
}

void OnlineNode::LoadModel(FILE *fp)
{
	fscanf(fp, "%d%d%d", &m_depth, &m_isLeaf, &m_label);
	fscanf(fp, "%lf%lf", &m_counter, &m_parentCounter);
	for (int i = 0; i < *m_numClasses; ++ i)
	{
		double v;
		fscanf(fp, "%lf", &v);
	...
	if (!m_isLeaf) {
		m_bestTest = new RandomTest(*m_numClasses, m_minFeatRange->rows(), *m_minFeatRange, *m_maxFeatRange);
		m_bestTest->LoadModel(fp);
		pair<VectorXd, VectorXd> parentStats = m_bestTest->getStats();
		m_rightChildNode = new OnlineNode(*m_hp, *m_numClasses, m_minFeatRange->rows(), *m_minFeatRange, *m_maxFeatRange, m_depth + 1,
			parentStats.first);
		m_rightChildNode->LoadModel(fp);
		m_leftChildNode = new OnlineNode(*m_hp, *m_numClasses, m_minFeatRange->rows(), *m_minFeatRange, *m_maxFeatRange, m_depth + 1,
			parentStats.second);
		m_leftChildNode->LoadModel(fp);
	} else {
		for (int nTest = 0; nTest < m_hp->numRandomTests; nTest++) {
			m_onlineTests[nTest]->LoadModel(fp);
		}
	}
}
//////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////
void OnlineTree::RecordModel(FILE *fp)
{
	printf("*****OnlineTree::RecordModel*****\n");
	m_rootNode->RecordModel(fp);
}

void OnlineTree::LoadModel(FILE *fp)
{
	printf("*****OnlineTree::LoadModel*****\n");
	m_rootNode->LoadModel(fp);
}
//////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////
void OnlineRF::RecordModel(FILE *fp)
{
	printf("*****OnlineRF::RecordModel*****\n");
	fprintf(fp, "%lf\t%lf\n", m_counter, m_oobe);
	for (int i = 0; i < m_hp->numTrees; ++ i)
	{
		m_trees[i]->RecordModel(fp);
	}	
}

void OnlineRF::LoadModel(FILE *fp)
{
	printf("*****OnlineRF::LoadModel*****\n");
	fscanf(fp, "%lf%lf", &m_counter, &m_oobe);
	for (int i = 0; i < m_hp->numTrees; ++ i)
	{
		m_trees[i]->LoadModel(fp);
	}	
}
//////////////////////////////////////////////////////////////////////////