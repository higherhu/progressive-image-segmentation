...
//===========================================================================
///	PerturbSeeds
/// 从8邻域中找出梯度（edge能量）最小的点，并将种子点移到该点。
//===========================================================================
	...
	const vector<double>&		edges)
{
	...
	for( int n = 0; n < numseeds; n++ )
	{
		...
		int storeind = oind;
		for( int i = 0; i < 8; i++ )	// 从8邻域中找出梯度（edge能量）最小的点，并将种子点移到该点。
		{
	...
	const string&				path) 
{
	...
	finalpath = path + temp + string(".txt");
...
void SLIC::PerformSuperpixelSegmentation_Fixed(
	vector<double>&				kseedsl,
	vector<double>&				kseedsa,
	vector<double>&				kseedsb,
	vector<double>&				kseedsx,
...
//===========================================================================
///	PerformSLICO_ForGivenSeeds
///
/// Zero parameter SLIC algorithm for a given number K of superpixels.
//===========================================================================
void SLIC::PerformSLICO_ForGivenSeeds(
	const unsigned int*			ubuff,
	const int					width,
	const int					height,
	int*						klabels,
	...
	vector<double>				kseedsy)
{
	assert(kseedsx.size() == kseedsy.size());
	vector<double> kseedsl(kseedsx.size());
	vector<double> kseedsa(kseedsx.size());
	vector<double> kseedsb(kseedsx.size());
	//--------------------------------------------------
	m_width  = width;
	m_height = height;
	int sz = m_width*m_height;
	...
	{
		int index = kseedsy[i]*width+kseedsx[i];
		kseedsl[i] = m_lvec[index];
		kseedsa[i] = m_avec[index];
		kseedsb[i] = m_bvec[index];
	}
	...
	int STEP = sqrt(double(sz)/double(K)) + 2.0;//adding a small value in the even the STEP size is too small.
	//PerformSuperpixelSLIC(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, klabels, STEP, edgemag, m);
	PerformSuperpixelSegmentation_Fixed(kseedsl,kseedsa,kseedsb,kseedsx,kseedsy,klabels,STEP,10);
	numlabels = kseedsl.size();

	int* nlabels = new int[sz];
	EnforceLabelConnectivity(klabels, m_width, m_height, nlabels, numlabels, 1000);
	{for(int i = 0; i < sz; i++ ) klabels[i] = nlabels[i];}
	if(nlabels) delete [] nlabels;
}